...
class Tests(unittest.TestCase):
    ...
    def test2(self):
        ...
        r = torch.randn(2, 3)
        output_kernel = kernel(r)

        radii = r.norm(2, dim=1)  # [batch]
        Y = kernel.sh(kernel.set_of_l_filters, r)  # [l_filter * m_filter, batch]
        R = kernel.R(radii)  # [batch, l_out * l_in * mul_out * mul_in * l_filter]
        norm_coef = kernel.norm_coef
        norm_coef = norm_coef[:, :, (radii == 0).type(torch.long)]  # [l_out, l_in, batch]
        ...
        output_kernel_fn = KernelFn.apply(*inputs)

        self.assertTrue(torch.allclose(output_kernel, output_kernel_fn))
